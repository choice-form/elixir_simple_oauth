defmodule SimpleOAuth.GoogleTest do
  ...
  describe "token/2" do
    test "returns {:ok, token}" do
      ...
      mock(fn %{method: :post, url: "https://oauth2.googleapis.com/token", body: ^request_body} ->
        %Tesla.Env{
          status: 200,
          body: %{
      ...
      assert {:ok, %{"access_token" => "access_token"}} = Google.token("code", test_config())
    end

    test "returns {:error, :get_google_token_error}" do
      mock(fn %{method: :post} -> %Tesla.Env{status: 500} end)
      assert {:error, :get_google_token_error} = Google.token("code", test_config())
    end
  end

  describe "user_info/2" do
    test "returns {:ok, user_info}" do
      mock(fn %{
                method: :get,
      ...
      assert {:ok, %{"email" => "test@gmail.com"}} = Google.user_info("access_token")
    end

    test "returns {:error, :get_google_user_info_error}" do
      mock(fn %{method: :get} -> %Tesla.Env{status: 500} end)

      assert {:error, :get_google_user_info_error} = Google.user_info("access_token")
    end
  end
